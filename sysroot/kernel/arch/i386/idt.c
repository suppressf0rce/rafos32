...
struct idt_entry {
  ...
  uint8_t  flags;
  uint16_t handler_hi;
} __attribute__((packed)); // Telling GCC not to optimize this struct
typedef struct idt_entry idt_entry_t;

/**
 * Defining special pointer for idt
 */
struct idt_ptr {
  uint16_t limit;
  uint32_t base;
} __attribute__((packed)); // Telling GCC not to optmize this struct
typedef struct idt_ptr idt_ptr_t;

// Declare an IDT entries and it's special pointer
idt_entry_t idt[IDT_NUM_ENTRIES];
idt_ptr_t idtp;

// Function arch/i386/idt_s.S, loads IDT from a pointer to an idt_ptr
